...
net.createServer((connection) => {
    ...
    connection.on('data', (data) => {
        const request = JSON.parse(data.toString()); // Parsear la solicitud del cliente
        let response;
        // Procesar la solicitud según el tipo
        switch (request.type) {
            case 'add':
                // Agregar un nuevo Funko
                const addSuccess = addFunko(request.funkoPop.id, request.funkoPop.usuario, request.funkoPop.nombre, request.funkoPop.descripcion, request.funkoPop.tipo, request.funkoPop.genero, request.funkoPop.franquicia, request.funkoPop.numero, request.funkoPop.exclusivo, request.funkoPop.caracteristicasEspeciales, request.funkoPop.valorMercado);
                response = {
                    type: 'add',
                    success: addSuccess,
            ...
            case 'remove':
                // Eliminar un Funko
                const removeSuccess = eliminarFunko(request.funkoPop.usuario, request.funkoPop.id);
                response = {
                    type: 'remove',
            ...
            case 'read':
                ...
                const funko = mostrarFunko(request.funkoPop.usuario, request.funkoPop.id);
                response = {
                    type: 'read',
                    success: funko !== null,
